...
class SVM:
    ...
    def set_parameters(self, iter = 10000000):
        self.iter = iter
    ...
    def _solve_primal(self):
        ...
        P = matrix(w_mask_m)
        q = matrix(np.zeros((params, 1)))
        G = matrix(-y * x)
        h = matrix(-np.ones((N, 1)))
        # solve QP
        # sol = solvers.qp(P, G, h)
